...
function webSocketUpgrade(req, socket, head) {
  this.webSocketServer.handleUpgrade(req, socket, head, ws => this.webSocketServer.emit('connection', ws, req))
}
/**
 * { function_description }
 *
...
function initialize(options) {
  ...
  if (options.http !== false) {
    ...
    this[httpServerSymbol].on('upgrade', this.webSocketUpgrade)
  }
  if (options.http2 !== false) {
    ...
    this[http2ServerSymbol].on('upgrade', this.webSocketUpgrade)
    ...
    this[httpsServerSymbol].on('upgrade', this.webSocketUpgrade)
  }
